...
long long init(vector<long long> &arr,vector<long long> &tree,int node,int start,int end){ 
    if(start==end) 
        ...
        return (tree[node] = init(arr,tree,2*node,start,(int)((start+end)/2)) + init(arr,tree,2*node+1,(int)((start+end)/2)+1,end) ); 
}
...
long long sum(vector<long long> &arr,vector<long long> &tree,int node,int start,int end,int left,int right){ 
    if(left > end || right < start) 
        ...
        return sum(arr,tree,2*node,start,(int)((start+end)/2),left,right) + sum(arr,tree,2*node+1,(int)((start+end)/2)+1,end,left,right);
}
...
int main(){
  ...
  for(int i=0;i<m+k;i++){
      ...
      if(a == 1){
        ...
        printf("%lld\n",sum(arr,segment_tree,1,0,n-1,b-1,c-1));
      }
