...
long long query(vector<long long> &tree, vector<long long> &lazy, int node, int start, int end, int x) {
    update_lazy(tree, lazy, node, start, end);
    if (x > end || x < start) {
        return 0;
    }
    ...
    long long lsum = query(tree, lazy, node*2, start, (start+end)/2, x);
    long long rsum = query(tree, lazy, node*2+1, (start+end)/2+1, end, x);
    return lsum + rsum;
}
...
int main() {
    ...
    while (m--) {
        ...
        if (what == 1) {
            ...
            int x;
            cin >> x;
            cout << query(tree,lazy, 1, 0, n-1, x-1) << '\n';
        }
    }
    return 0;
}
